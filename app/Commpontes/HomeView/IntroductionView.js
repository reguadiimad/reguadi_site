...
const TypingAnimation = ({onComplete}) => {
  ...
  useEffect(()=>{if(phase>=3)onComplete(true)},[phase])

  const ThePhase2 = ({ outlined = false }) => (
    <motion.div className={`ease-in-out duration-150  font-bold text-center text-gray-900 dark:text-gray-100 ${outlined && " hd "} ${isArabic ? 'text-8xl -mr-[10%] sm:mr-0 sm:text-9xl lg:text-[150px] xl:text-[180px] my-5 sm:my-10 xl:scale-110 font-arb2' : 'text-[90px] leading-18 sm:leading-relaxed md:text-8xl lg:text-9xl  xl:text-[150px] xl:scale-125 font-clashDisplay'}`}>
      {phase === 2 ? (
  ...
  return (
    <motion.div className={`flex flex-col items-center mt-2 md:mt-5 w-full px-4 h-auto ${isArabic ? 'font-arb' : ''}`} layout dir={isArabic ? "rtl" : "ltr"} id='typing'>
      ...
      {phase >= 2 && (
        <motion.div dir={'ltr'} layout className="mt-4 relative flex items-center justify-center">
          <ThePhase2 />
          ...
          {
            phase >=2 && (
               <div className=' h-[120%] -top-[10%]   w-full md:w-[110%] left-0 md:-left-[5%] xl:w-[130%] xl:-left-[15%] absolute pointer-events-none flex flex-col sm:flex-row items-center justify-center'>
                <div transition={{ duration: 0.5 }} className='w-full h-[50%] sm:w-[50%] sm:h-full flex items-center'>
                  <CrystalToggle/>
                </div>
                <div className='w-full h-[50%] sm:w-[50%] sm:h-full flex items-center'>
                  <BackendGearsDisplay/>
                </div>
              </div>
            )
